  ...
  return <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[1000] overflow-y-auto py-6 px-4">
    <div className="bg-white rounded-xl w-full max-w-[560px] max-h-[90vh] overflow-y-auto shadow-[0_10px_30px_rgba(0,0,0,0.15)] p-6 text-left">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-[#eee]"><button type="button" onClick={onClose} className="py-2.5 px-6 rounded-md bg-white text-[#555] border border-gray-300">Cancel</button><button type="submit" disabled={submitting} className="py-2.5 px-6 rounded-md bg-[#1e3c72] text-white disabled:opacity-60">{submitting ? 'Submitting…' : (isFollowUp ? 'Submit Follow-Up' : 'Submit Request')}</button></div>
      </form>
    </div>
  </div>;
}
